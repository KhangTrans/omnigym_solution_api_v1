...
router.get('/', isAuthenticated, authorizeRole(['Admin', 'Staff', 'BranchManager']), getUsers);
router.post('/', isAuthenticated, authorizeRole(['Admin']), createUser);

// Admin và Staff được khóa/mở khóa tài khoản
router.patch('/:id/status', isAuthenticated, authorizeRole(['Admin', 'Staff', 'BranchManager']), updateUserStatusHandler);
